...
grid_search_lr = GridSearchCV(estimator=LogisticRegression(max_iter=10000, random_state=1),
...
# Initialize Models & Classifier -  With parameters determined by best_params_*

logistic_reg = LogisticRegression(C = .01,
                                  penalty = 'l1',
                                  solver = 'liblinear',
                                  max_iter=10000, 
                                  random_state=1)
random_forest = RandomForestClassifier(n_estimators=200,
                                       ...
                                       bootstrap= False,
                                       random_state=1)
gradient_boosting = GradientBoostingClassifier(subsample = .8,
                                               n_estimators=100,
                                               min_samples_split=5,
                                               min_samples_leaf=2,
                                               max_depth=3,
...
print("Bets Made: " + str((test_moneyline.my_bet_lost.sum() + test_moneyline.my_bet_won.sum())))
print("Possible Games: " + str(len(test_moneyline)))


## Display Results as Dataframe

results_df = test_moneyline.groupby(['schedule_season', 'schedule_week']).agg({
...
final_df = results_df.append({
    'schedule_season' : '2022',
    'schedule_week' : 'Total',
    'bet_accuracy' : "{:.2f}".format(test_moneyline.my_bet_won.sum() / (test_moneyline.my_bet_lost.sum() + test_moneyline.my_bet_won.sum()) * 100),
    'total_bets' : test_moneyline.my_bet_lost.sum() + test_moneyline.my_bet_won.sum(),
    'total_games' : len(test_moneyline)
}, ignore_index=True)
